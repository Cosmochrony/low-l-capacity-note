...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\textwidth]{../code/convergence_8_3_S3.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{../code/lowl_capacity_grid.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../code/lowl_capacity_grid_pol.pdf}
...
beyond its first pass; whether the observed spectrum saturates the capacity ceiling.
A first pre-registered TT extremum-position search for the $\sqrt{8/3}$ modulation ratio found no
qualifying pair at the declared threshold; this constrains the naive angular realisation of the
witness, not the underlying $S^3$ fibre invariant.
...
then seventeen.
These numbers are not free either: they obey the exact recurrence
\[
\Nhist(n)=2\,\Nhist(n-1)+\Nhist(n-2),
\]
whose structural reading is carried by the trajectory-branching note~\cite{Beau2026tb}: the
projected branching is not a memoryless doubling.
The leading factor $2\,\Nhist(n-1)$ is the minimal trace of the binary non-injective continuation,
while the term $\Nhist(n-2)$ records the residual contribution of the previous, not yet fully
absorbed layer, so the distinguishable-history channel has effective memory depth one.
On this reading the universe does not simply double its possibilities at each rank; it doubles with
a one-step memory, and this is what makes the unfolding structured rather than a bare binary tree.
...
The same small counts carry a thermal consequence: the beginning of the observable universe is
described, within this interpretation, as a state of finite resolution rather than of infinite
temperature.
Temperature is then an effective parameter, meaningful only once the projective unfolding has
produced enough distinguishable degrees of freedom to carry a statistical description; the hot
classical regime is untouched, while the extrapolation to an infinitely hot initial state is
...
rate~\cite{Beau2026tb}; the compression of full-history profiles, once a candidate for a slowly
evolving effective equation of state, is now gated by a conditional no-go that confines its
observable content to the early low-rank window~\cite{Beau2026tb}.
