...
TE is neutral and EE mildly disfavoured, as expected from the missing transfer treatment.
...
convergence is monotone from $N=10^4$ onward (Figure~\ref{fig:witness}).
...
as a high-$N$ mode.
The open theoretical step is the following: why does a small distinguishable-history number
suppress ensemble power, rather than only enlarge the cosmic variance~\cite{Knox1995}?
The proposed answer is that history capacity bounds the entropy of the mode: a mode whose
projective support contains only $N$ distinguishable histories cannot carry the same number of
independent amplitude realisations as the high-rank effective ensemble, so the observed large-angle
power is biased toward a bounded-capacity estimate rather than merely scattered around the
unconstrained $\lcdm$ mean.
This entropy-to-power step is the central theoretical point to sharpen; the numerical section below
tests its two minimal statistical conversions without attempting to decide it.
...
Second, TE is neutral to mildly favourable and EE mildly disfavoured
($\Delta\chi^2\simeq+1$); this is consistent with the minimal propagation ignoring the
reionisation-dominated low-$\ell$ EE transfer, and is reported as an open tension of the minimal
version, not hidden.
...
\section{Suggested replacement for the corpus text}
\label{sec:replacement}

The following paragraph can replace the phenomenological low-$\ell$ claim of the white-paper
appendix~\cite{Cosmochrony}, and sharpens the pre-branching wording of the cosmology
paper~\cite{Beau2026d}.

\begin{quote}
At the largest angular scales, the relevant modes probe the first admissibility ranks of the
projective description.
The suppression mechanism is therefore not an adjustable dynamical damping term.
It follows from the small absolute number of distinguishable projective histories available at
those ranks.
In the canonical counting channel this number is
$\Nhist(n)=((1+\sqrt{2})^{n+1}+(1-\sqrt{2})^{n+1})/2$, so the first ranks contain only $3$, $7$,
and $17$ distinguishable histories.
The resulting large-angle deficit is a small-$N$ distinguishable-history effect.
The comparison with CMB data must be made by applying the derived history envelope to the $\lcdm$
spectrum, not by attenuating the observed Planck points.
\end{quote}
...
Open: the entropy-to-power step (why bounded history capacity biases ensemble power rather than
only enlarging cosmic variance); the transfer treatment of TE/EE; the modulation-ratio search
($\ell_2/\ell_1\simeq1.633$); the corpus text updates (white-paper appendix and cosmology
pre-branching wording).
